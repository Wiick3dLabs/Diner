...
app.post('/dishes', function(req, res) {
	db.run("INSERT INTO dishes (name, price, image, category_id) VALUES (?,?,?,?)", req.body.name, req.body.price, req.body.image, req.body.category_id, function(err) {
		if(err) {
			throw err;
		}
    var id = this.lastID;
...
app.put('/dishes/:id', function(req, res) {
	var id = req.params.id;
	db.run("UPDATE dishes SET name = ?, image = ?, price = ?, category_id = ? WHERE id = ?", req.body.name, req.body.image, req.body.price, req.body.category_id, id, function (err) {
		if(err) {
			throw err;
		}
		db.get("SELECT * FROM dishes WHERE id = ?", id, function(err, row) {
